...
export default function MenuButton({ icon, title, detail, onClick, accent, compact, className = '' }) {
  if (compact) {
    return (
      <button
        onClick={onClick}
        className={`group relative flex items-center bg-slate-50 border-2 border-slate-400 hover:border-slate-500 px-5 py-4 rounded-xl transition-all duration-300 hover:shadow-md shadow-sm min-h-[72px] ${accentColors[accent]} hover:-translate-y-0.5 ${className}`.trim()}
      >
        <div className="absolute inset-0 bg-gradient-to-br from-slate-50/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity rounded-xl" />
        <div className="relative flex items-center gap-3 w-full min-w-0">
          <span className="text-2xl transform group-hover:scale-105 transition-transform shrink-0 leading-none">{icon}</span>
          ...
          <svg className="w-5 h-5 text-slate-600 group-hover:text-slate-800 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
